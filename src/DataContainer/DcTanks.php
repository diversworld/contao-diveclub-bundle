...
/*
    ...
    public function __invoke(array $arrButtons, DataContainer $dc): array
    {
        if ('edit' === Input::get('act')) {
            $arrButtons['createInvoice'] = '<button type="submit" name="createInvoice" id="createInvoice" class="tl_submit createInvoice" accesskey="x">'.$GLOBALS['TL_LANG']['tl_dc_tanks']['createInvoiceButton'].'</button>';
        }
    ...
    #[AsCallback(table: 'tl_dc_tanks', target: 'config.onsubmit', priority: 100)]
    public function runCreateInvoice(): void
    {
        if ('' !== Input::get('id') && '' === Input::post('createInvoice') && 'tl_dc_tanks' === Input::post('FORM_SUBMIT') && 'auto' !== Input::post('SUBMIT_TYPE')) {
    ...
    public function customButtonEvent()
    {
        ...
        if ($tankId)
        {
            ...
            // Get member, pid from tl_dc_tanks
            $result = $db->prepare("SELECT member, pid, size FROM tl_dc_tanks WHERE id = ?")
                ->execute($tankId);
            ...
            $query = "INSERT INTO tl_dc_check_invoice (title, alias, tstamp, pid, checkId, member, published, invoiceArticles, priceTotal)
                      VALUES (?, ?, ?, ?, ?, ?, 1, ?, ?)";
