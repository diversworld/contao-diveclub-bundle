# Willkommen im ContaoDiveclubBundle Wiki

Dieses Bundle ermöglicht Tauchvereinen und -schulen die effiziente Verwaltung von Tauchausrüstung, Reglerservicing,
TÜV-Prüfungen für Tauchflaschen sowie die Organisation von Tauchkursen.

## Inhaltsverzeichnis

1. **[Installation & Konfiguration](Installation-und-Konfiguration)**
    - Systemvoraussetzungen
   - Ersteinrichtung und globale Einstellungen
   - Frontend-Seiten und Weiterleitungen
   - Tabellenstruktur
2. **[Ausrüstungsverwaltung](Ausruestung-und-Regler)**
    - Verwaltung von Leihausrüstung
    - Regler-Service und Historie
3. **[TÜV-Management & Flaschen](TUEV-und-Flaschen)**
    - Verwaltung vereinseigener Flaschen
    - Planung von TÜV-Sammelterminen
    - Buchungsprozess für Mitglieder
4. **[Equipment-Verleih](Equipment-Verleih)**
    - Ablauf von Reservierung bis Rückgabe
    - Status-Definitionen
5. **[Kursverwaltung](Kursverwaltung)**
    - Erstellung von Kursen und Modulen
    - Fortschrittskontrolle für Schüler
    - Kurskalender
6. **[Anpassungen & Templates](Templates)**
    - Verwendung von Insert-Tags
    - Anpassung der Twig-Templates

---
[Zurück zur Repository-Übersicht](https://github.com/diversworld/contao-diveclub-bundle)
